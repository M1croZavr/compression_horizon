...
\begin{itemize}
    ...
    \item An empirical finding that cramming operates through attention hijacking, with compression tokens capturing 40-80\% of attention mass in fixed intermediate layers
    \item Evidence that reconstruction success does not imply preserved downstream capabilities, establishing a distinction between pixel-level recovery and semantic encoding
\end{itemize}
...
exclusively occur at token indices 0 and 1, while later positions are typically
...
Because autoregressive decoding is brittle to early errors (Section~\ref{sec:prior_limitations}), this comparison should be interpreted primarily through the lens of \emph{perfect} reconstruction: progressive cramming trades a fixed-budget constraint for a fixed-accuracy constraint. Despite current table shows  progressive cramming in current setup underperforms full cramming in terms of both information gain and compressed tokens, in section~\ref{sec:activation_alignment} we will show that with some regularizations progressive cramming may lead to comparable results with full cramming achieving $5103 \pm 748$ information gain - that is comparable and even sightly better than information gain $4865.7\pm546.6$ reported in original cramming paper by \citet{kuratov2025cramming}.
...
Across model families, the trajectories are well-approximated by surprisingly low-dimensional structure: only a modest number of principal components explains 99\% of the trajectory variance, even when hundreds (or thousands) of tokens are reconstructed (Table~\ref{tab:all_progressive_modifications}). Table~\ref{tab:all_progressive_modifications} further suggests that Gemma~3 achieves noticeably lower information gain at smaller reconstructed lengths than Llama and Pythia, consistent with additional optimization constraints such as logits softcapping \cite{gemma2}.


{\color{red} todo develop this statement about gemma and softcapping?}

We visualized one trajectory for progressive cramming on two PCA components at Figure~\ref{fig:visual_abstract_optimization_trajectory_with_good_accuracy}. Colors encodes 90\%+ accuracy reconstruction (with teacher-forcing) area for corresponding points. On the figure we may see that token index increases, area of high-accuracy reconstruction decreases. And its harder and harder to find suitable directions when sequence length increases.
...
% TODO may be make image shorter in heigth?
\begin{figure}[t]
    \centering
...
Despite table~\ref{tab:all_progressive_modifications} shows that for Llama-3.1-8B about 74 PCA components explains 99\% of variance, we may see that it takes at least twice more components to achieve near-perfect accuracy with teacher-forcing.
We also observe the same failure mode as in the full cramming setup (Section~\ref{sec:prior_limitations}):
errors occur at the start of the sequence, which breaks further decoding from the reconstructed embedding, and many errors appear in the first decoded token. This only happens if reconstruction accuracy is lower than 100\%. For perfect reconstruction there is no such problem.

% {\color{red} TODO update plot and description}

\begin{figure}[t]
    \centering
    \centerline{\includegraphics[width=\columnwidth]{aggregate_pca_reconstruction_accuracy.pdf}}
    \caption{Reconstruction accuracy for different numbers of PCA components for Llama-3.1-8B.}
    \label{fig:pca_reconstruction_accuracy}
\end{figure}

\subsection{Activation Alignment}\label{sec:activation_alignment}

Table~\ref{tab:all_progressive_modifications} studies activation alignment (Section~\ref{sec:activation_alignment}) and its interaction with low-dimensional projection.
...
We measure attention mass using the definition in Section~\ref{sec:attention_mass}, comparing attention to position 0 in the crammed setting (compression embedding) to attention to the BOS token in the uncompressed baseline.

Table~\ref{tab:attn_hijacking} shows that attention hijacking is highly model-dependent when aggregated over layers.
SmolLM2 and Pythia exhibits strong correlation between BOS and compression-embedding attention profiles: the compression embedding takes more attention mass on the same layers as the BOS token does. Pythia often assigns comparable or even higher average attention mass to the compression embedding than to BOS. This may mean than the compression embedding works similarly to the BOS token: it works like attention sink, but rather helping model, its hardly overrides generation and breaks language modeling abilities (Section~\ref{sec:semantic_evaluation}) - thats why we called this attention pattern hijacking: it breaks model.
...
We next hypothesize that if the compression embedding and the BOS token are similar in phenomenon of attention sink it may be a problem to have in a sequence both a compression embedding and a BOS token due to possible conflict for the same characteristics for embeddings in hidden states.

It also may be hard to force model to generate BOS token due to it never generates it during normal generation.

And considered experiments with removed BOS token. We compare progressive cramming with and without BOS token in Table~\ref{tab:progressive_no_bos_token}.
Removing BOS token improved crammed tokens count, information gain and reduced attention mass on compression embedding only for Llama-3.1-8B model. And it has no effect for all other models neither in terms of information gain, nor any effect on attention mass.
...
\begin{table}[]
    \centering
    \caption{Progressive cramming without BOS token.}
...
\begin{table}[t]
    \centering
    \caption{Downstream multiple-choice evaluation on HellaSwag and ARC-Easy (4-way; chance = 25\%). ``Base'' uses the original prefix; ``Cram'' prepends an optimized compression embedding. \bcancel{B} symbol stands for experiments with removed BOS token.}
...
Table~\ref{tab:semantic_evaluation} shows that prepending the crammed compression embedding causes a large and consistent drop in downstream accuracy across model families.
In most settings, performance collapses to (or below) the random-guessing baseline, indicating a failure mode stronger than ordinary degradation: the compression embedding breaks the model's ability to use the prefix to score candidate suffixes.
...
\section{Progressive actoss models scales}
...
\toprule
Model & Tokens & Setup & Final conv. & Greedy conv. & Mismatch@0 (\%) & Mismatch@1 (\%) & Mismatch@2 (\%) \\
\midrule
...
\midrule
Meta-Llama-3.1-8B & 1568 & common    & -- & -- & -- & -- & -- \\
Meta-Llama-3.1-8B & 1568 & no BOS    & -- & -- & -- & -- & -- \\
Meta-Llama-3.1-8B & 1568 & 2 leading & -- & -- & -- & -- & -- \\
\bottomrule
\end{tabular}
\end{table*}
