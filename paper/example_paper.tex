...
\begin{abstract}
    ...
    On likelihood-based multiple-choice evaluation (HellaSwag, ARC-Easy), prepending a crammed compression embedding (optimized for prefix reconstruction, but not guaranteed to be perfect for every benchmark instance) reduces accuracy to close to random guessing across model families, even when the original prefix remains in context.
...
Understanding this mechanism matters beyond cramming itself; it can shed light on how transformers process information stored in their embedding space.
...
\begin{table}[t]
    \centering
    \caption{Downstream multiple-choice evaluation on HellaSwag and ARC-Easy (4-way; chance = 25\%). ``Base'' uses the original prefix; ``Cram'' prepends a per-instance optimized compression embedding (reconstruction is targeted but not guaranteed to be perfect). \bcancel{B} denotes experiments with the BOS token removed.}
