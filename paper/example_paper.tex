...
{\color{red}
    ...
    The Last 1\% Problem. Show example of how 1 early error cascades in autoregressive decoding. Maybe a figure?
...
Rather than compressing a fixed sequence, we progressively extend the target:
%
\begin{enumerate}
    \item Initialize with $\mathbf{x}^{(1)} = (x_1)$, optimize $\mathbf{e}^{(1)}$ to 100\% accuracy
    \item Extend to $\mathbf{x}^{(k)} = (x_1, \ldots, x_k)$, keep optimization from $\mathbf{e}^{(k-1)}$ point.
    \item Continue until convergence fails
\end{enumerate}

This process is visualized on Figure~\ref{fig:progressive}.

At the end of optimization process we have sequence of compression embeddings $ e^{(1)} \dots e^{(n)} $ for each token. With this sequence we can track optimizer path for each new token. And get aproximate trajectory length of optimization process. We define it as sum of euclidian dictances between sequential pairs of compression embeddings.

\begin{equation}
    L = \sum^{n-1}_{i=1}{ \| e^{(i+1)} - e^{(i)} \| }
\end{equation}
...
\begin{figure*}[t]
    ...
    \caption{Progressive cramming sequentially adds tokens, optimizing to 100\% accuracy at each stage before extending. All tokens are optimized during each stage, not only last one.}
...
Let $\mathbf{h}_l^{\text{cram}}(i)$ denote the hidden state at layer $l$, position $i$ with compression embedding, and $\mathbf{h}_l^{\text{clean}}(i)$ the corresponding state without compression.
The activation alignment loss is based on cosine distance:
%
\begin{equation}
...
where $K$ is number of layers for alignment.

The full training objective for activation allignment combines reconstruction and alignment:
...
where $\mathbf{z} \in \mathbb{R}^k$ with $k \ll d$, and $\mathbf{W} \in \mathbb{R}^{d \times k}$. $\mathbf{z}$ and $\mathbf{W}$ $\mathbf{b}$ are all learned and randomly initialized for each compressed sample.

This is simple linear projection and after training it can be computed only once and saved as high-dimensional embedding. But there a difference with single embedding optimization: linear projection makes optimizer to have an moments for this projection.

\subsection{Information Gain}

As a main metric for cramming task we use Information Gain. Let $H_{LM} = H(P_{\theta}(t_{[1:N]}))$ be the sum of per token cross entropy in bits for original tokens sequence without compression embedding. And $H_{[mem]+LM} = H(P_{\theta}(t_{[1:N]}|\texttt{[mem]}))$ be the sum of per token cross entropy in bits for crammed tokens sequence.

\begin{equation}
    C_H = H_{LM} - H_{[mem]+LM}
\end{equation}

\citet{kuratov2025cramming} showed that cramming is limited not with tokens number rather with information gain and information gain for models remains stable for different types of datasets.
...
We used following models families for cramming: Pythia \cite{biderman2023pythia}, Llama3 \cite{dubey2024llama3} SmolLM2 \cite{allal2025smollm2} and Gemma3 \cite{gemma3}. For experiments we considered PG19 dataset \cite{Rae2020CompressiveTransformerPG19}.

Experiments shows that progressive experimental setup performs on par with original full cramming setup. Table~\ref{tab:full_vs_progressive} shows that progressive cramming achieves perfect accuracy that is crucial for autoregressive reconstruction, but degrades a bit in terms of information gain and comressed tokens. However as shown in section~\ref{sec:prior_limitations} non-perfect accuracy is crucial for reconstruction.

\begin{table}[]
    \centering
    \caption{Full and progressive cramming comparison. Full cramming corresponds to original cramming proposed by \citet{kuratov2025cramming}. Progressive cramming is ours one. For progressive cramming we have fixed accuracy and varying tokens count. While for full cramming we have fixed tokens count and varying accuracy.}\label{tab:full_vs_progressive}
...
We collect embeddings $\{\mathbf{e}^{(k)}\}_{k=1}^{n}$ and analyze their structure. PCA analysis reveals that 99\% of variance is explained by low-dimensional subspace. Number of PCA components that explains 99\% of accuracy grows logarithmically with sequence length increases as shown on figure~\ref{fig:pca_from_seq_length}. This observation is supported for different models families as shown in table~\ref{tab:pca_models}. Larger models utilized more internal directions. Gemma3 models are difficult to compress - it shows similar information gains across different models sizes and it is times lower compared to models from other families. This happens most probably due to logits softcapping intoduces by \citet{gemma2} to avoid logits to become very large.
...
\begin{table*}[]
    \centering
    \caption{Progressive cramming optimization trajectories metrics. Compressed tokens - is number of compressed tokens in PG19 dataset.}
...
\begin{figure}[t]
    ...
    \caption{Dependence of the number of PCA components that explains 99\% optimization trajectory variance for Llama-3.1-8B and different learning rate values.}
    \label{fig:pca_from_seq_length}
\end{figure}

\subsection{Dataset Modifications}

We also visualized pairs of 4 PCA components for progressive cramming trajectories at figure~\ref{fig:trajectories_text_modifications}. For each sample first 64 tokens were the same for all text modifications. We draw projections for following modifications of suffix: random words permutations, suffix generated by Llama-3.1-8B, characters lowercase. For all trajectories points corresponding to common prefix are match. After 64 token all trajectories goes different paths. And nevertheless base and lowercased sequences has similar semantics, they have very different trajectories.



{\color{red} TODO add tables with text modifications to appendix showing that model-sampled sequences are easy to compress}

\begin{figure*}[t]
    \centering
    \centerline{\includegraphics[width=\linewidth]{Llama3.1-8B-text-modifications_lr-0p1.pdf}}
    \caption{Visualization of progressive embeddings for 4 PCA components. For first sample of PG19 for Llama-3.1-8B. All compressed sequences has same 64 tokens prefix. Base trajectory corresponds to original PG19 sample. For random trajectory we have random permutation for all tokens after 64 tokens prefix. "Sampled" trajectory corresponds to tokens sampled with greedy decoding with the same model Llama-3.1-8B conditioned on 64 tokens prefix. For lowercase trajectory we applied lowercase transform for all characters after 64 tokens prefix - this transform should not change text semantics, but makes different tokens to be compressed. For each component there is a signed fraction of explained variance at the axis title. Start point id marked as a circle, end points are marked as the squares.}
...
\begin{table}[]
    \centering
    \caption{Progressive Cramming metrics for modified samples from PG19. Sequences has 64 common tokens at the start. {\color{red} TODO \texttt{scripts/paper/text\_modifications\_table.py} recompute tab:text\_modifications script. Metrics are provided for Llama-3.1-8B. }}
...
We found that optimization with low-dim projection makes optimization:
\begin{itemize}
    \item More stable: Training gets less sensitive to learning rate;
    \item It reduce intrisic dimention of optimization trajectory with usually better results.
\end{itemize}

Table~\ref{tab:low_dim_projection_results} shows experiments on different models and different projection sizes. The most initeresting insights from this table:
\begin{itemize}
    \item Increasing initial dimension of linear projection does not increases PCA99\% components, but do increases optimization trajectory lenght
    \item Low dimensional projection for some models (Llama3 and SmollM2) it shifts the limits of compression compared to progressive optimization without low dimensional projection
\end{itemize}

\begin{table*}[]
    \centering
    \caption{Low dimention projection sutup.}
...
Table~\ref{tab:alignment_and_lowdim_projection} shows results for activations alignment. And again for smalllm2 and pythia it shifts the limits of compression as in terms of tokens as in terms if information gain even higher then was reported in \citet{kuratov2025cramming} (it was 4865 \pm 546 information gain for Llama-3.1-8B and 792 \pm 143 for pythia-1.4b).

Activation alignment helps to maintain original sequence hidden states and shifts attention hijacking to deeper layers.

\begin{table*}[]
    \centering
    \caption{Activation alignment and low dimensional projection}
...
We analyze attention patterns to understand how compression embeddings achieve reconstruction. We found that for Llama and SmolLM2 models compression token receives comparable attention mass as BOS token with high per-layer correlation. However for pythia and gemma models there was no correlation in compressed token attention mass and bos token attention mass.

{\color{red} TODO attention mass equation to method section}


\begin{table*}[]
    \centering
    \caption{Attention hijacking: average attention mass for compression and bos token over all layers}
...
Table~\ref{tab:prefix_tuning_attention_hijacking} shows that for some models it

\begin{table*}[]
    \centering
    \caption{Attention hijacking for prefix tuning}
...
\begin{table}[]
    \centering
    \caption{Prefix tuning accuracy compared to base progressive training results}
...
\paragraph{Setup.}
We evaluate on HellaSwag and ARC with:
\begin{enumerate}
    \item Original prefix (baseline)
    \item \texttt{[mem]} + Original prefix
\end{enumerate}

For the second option there was separate `[mem]` token per sample optimized for ideal prefix reconstruction. Each benchmark has 4 continuations for each sample.  Model choice was selected based on the lowest continuation perplexity normalized on suffix tokens count. Results are presented in table~\ref{tab:semantic_evaluation}. All model checkpoints are failed to choose correct suffix in benchmarks and this does not depend on predicted model tokens capacity. This makes us to conclude that cramming task looses prefix semantics and breaks language modeling capabilities.

\paragraph{Results.}
\begin{table}[t]
    \centering
    \caption{Downstream task performance. Compressed prefixes cause complete capability collapse. HS - HellaSwag, ARC-E - ARC-Easy, ARC-C - ARC-Challenge {\color{red} TODO recalculate on 512 samples each checkpoint}}
...
Models fail entirely—even with decompressed tokens in context.
This confirms that compression tokens override rather than inform model computation.
...
\begin{table*}
    \caption{Full experiments list with activations alignment and low dim projections}\label{tab:full_activation_alignment_and_low_dim_projections}
