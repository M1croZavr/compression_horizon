...
SmolLM2 and Pythia exhibits strong correlation between BOS and \texttt{[mem]} attention profiles: compression token takes more attention mass on the same layers as BOS token does. Pythia often assigns comparable or even higher average attention mass to \texttt{[mem]} than to BOS. This may mean than \texttt{[mem]} token works similary to BOS token: it works like attention sink, but rather helping model, its hardly overrides generation and breaks language modeling abilities (Section~\ref{sec:semantic_evaluation}) - thats why we called this attention pattern hijacking: it breaks model.
...
We next hypothesize that if compression token and BOS tokens are similar in phenomenon of attention sink it may be a problem to have in a sequence both an compression embedding and a BOS token due to possible conflict for the same characteristics for embeddings in hidden states.
...
\begin{table}[t]
    \centering
    \caption{Downstream multiple-choice evaluation on HellaSwag and ARC-Easy (4-way; chance = 25\%). ``Base'' uses the original prefix; ``Cram'' prepends an optimized \texttt{[mem]} embedding. \bcancel{B} symbol stands for experiments with removed BOS token.}
...
\begin{table*}[]
    \centering
    \caption{Prefix-tuning attention mass, computed with the same metric as Table~\ref{tab:attn_hijacking} but for learned prefix tokens instead of \texttt{[mem]}.}
