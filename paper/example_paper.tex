...
\begin{figure}[t]
    \centering
    \centerline{\includegraphics[width=\columnwidth]{pca_reconstruction_accuracy.pdf}}
...
\begin{table}[t]
    \centering
    \caption{Downstream multiple-choice evaluation on HellaSwag and ARC-Easy (4-way; chance = 25\%). ``Base'' uses the original prefix; ``Cram'' prepends an optimized \texttt{[mem]} embedding.}
    ...
    \begin{tabular}{lllll}
        ...
        \hline
         ...
         \small SmolLM2-1.7B  & 51.54{\small \%} & 23.17{\small \%} & 58.77{\small \%} & 30.69{\small \%} \\
         \small Llama-3.1-8B  & 40.79{\small \%} & 23.24{\small \%} & 38.48{\small \%} & 27.87{\small \%} \\
         \small gemma-3-4b-pt & 29.27{\small \%} & 26.18{\small \%} & 39.70{\small \%} & 24.29{\small \%} \\
        \hline
    \end{tabular}
...
{\color{red} TODO what if we remove bos token? will it be easier  to compress? Add Llama Results last exp to tab:progressive_no_bos_token}

{\color{red} how no bos token affects semantic evaluation?}

{\color{red} how much bos token is somilar to bos token?}
