...
Across model families, the trajectories are well-approximated by surprisingly low-dimensional structure: only a modest number of principal components explains 99\% of the trajectory variance, even when hundreds (or thousands) of tokens are reconstructed (Table~\ref{tab:all_progressive_modifications}).
...
Table~\ref{tab:all_progressive_modifications} further suggests that Gemma~3 achieves noticeably lower information gain at smaller reconstructed lengths than Llama and Pythia, consistent with additional optimization constraints such as logits softcapping \cite{gemma2}.
...
\begin{itemize}
    ...
    \item For Pythia-1.4B, projection can reduce token count and information gain relative to the baseline; combining projection with activation alignment partially recovers performance compared to projection alone. For Gemma in our setting, projection reduces token count and information gain relative to the baseline.
\end{itemize}
...
Reconstruction accuracy saturates quickly; 15 components already yield near-perfect recovery.
However, we observe the same failure mode as in the full cramming setup (Section~\ref{sec:prior_limitations}):
errors occur at the start of the sequence, which breaks further decoding from the reconstructed embedding, and many errors appear in the first decoded token.

% TODO maybe make image shorter in heigth?
\begin{figure}[t]
    \centering
...
We next hypotesize that if compression token and BOS tokens are similar in phenomenon of attention sink it may be a problem to have in a sequence both an compression embedding and a BOS token.
...
Removing BOS token had no significant effect on the compressed token count and information gain.


{\color{red} TODO move this table to appendix? }

\begin{table*}[]
    \centering
    \caption{Attention hijacking summary. ``Compression Token'' is attention mass to position 0 for \texttt{[mem]} in the crammed input; ``BOS Token'' is attention mass to position 0 for the uncompressed baseline; ``Diff'' is (Compression$-$BOS). ``Correlation'' is the Pearson correlation between per-layer attention-mass profiles.}
...
{\color{red} TODO what if we remove bos token? will it be easier  to compress? Add Llama Results last exp to tab:progressive\_no\_bos\_token}

% {\color{red} how no bos token affects semantic evaluation?} - done
...
We used 10 samples from PG19 for most experiments. Across all settings, the base language model was frozen; only the compression-token parameters were optimized (either the compression embeddings, or the low-dimensional embedding together with its projection). We optimized with AdamW. For each model, we swept the learning rate over \(\{0.01, 0.1, 0.5, 1.0\}\); the resulting metrics are summarized in Table~\ref{tab:all_learning_rates}. For the low-dimensional embedding and projection variant, we used a learning rate of 0.01. We ran up to 10{,}000 optimization steps per sample in all experiments; for progressive runs, we additionally capped the optimization of each newly added token at 1{,}000 steps. We used 100 warmup steps for all models. Unless stated otherwise, we used AdamW optimizer with $\beta_1=0.9$ and $\beta_2=0.9$, \texttt{weight\_decay}=0.01, \texttt{max\_grad\_norm}=1.0, \texttt{lr\_scheduler\_type}=\texttt{cosine\_with\_min\_lr} with \texttt{min\_lr}: $10^{-3}$, and \texttt{dtype}=\texttt{bf16}.
