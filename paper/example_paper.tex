...
Our study remains limited in scope: we evaluate a small set of model families and checkpoints and use a small number of PG19 samples, and our mechanistic probes are correlational rather than causal.
Future work should broaden model coverage (including additional architectures and instruction-tuned variants), strengthen causal analysis via targeted interventions, and develop a \emph{fast} compressor that produces semantically meaningful compression embeddings (ideally retaining factual precision) for retrieval-style workloads such as needle-in-a-haystack benchmarks, without per-example optimization.
