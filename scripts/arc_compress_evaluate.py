"""Evaluate compression on ARC benchmark.

This script:
1. Downloads ARC dataset (AI2 Reasoning Challenge)
2. Compresses the question (prefix) for each item
3. Evaluates by computing PPL for all answer choices and choosing the one with lowest PPL
...
def compute_ppl_baseline_batch(
    ...
    Args:
        ...
        tokenizer: Tokenizer
        context: Context text (question)
        endings: List of ending texts (answer choices)
        device: Device to use
...
def answer_key_to_index(answer_key: str, labels: list[str]) -> int:
    """Convert answer key (e.g., 'A', 'B', 'C', 'D') to index.

    Args:
        answer_key: The answer key string (e.g., 'A', 'B', 'C', 'D', or '1', '2', '3', '4')
        labels: List of label strings from the dataset
    ...
    # Fallback for numeric answer keys
...
def main():
    ...
    parser.add_argument(
        ...
        help="ARC subset to evaluate (ARC-Easy or ARC-Challenge)",
    )
    parser.add_argument(
        "--limit_samples",
        type=int,
    ...
    # Load ARC dataset
    print(f"Loading ARC dataset ({args.arc_subset})...")
    # ARC dataset structure:
    ...
    for batch_idx in tqdm(range(num_batches), desc="Processing batches"):
        ...
        for sample_idx in range(current_batch_size):
            ...
            result = {
                "sample_id": idx,
                "arc_id": batch_items[sample_idx]["id"],
                "context": context,
                "endings": endings,
                "label": label,
                "answer_key": batch_items[sample_idx]["answerKey"],
    ...
    output_data = {
        ...
        # 1. Baseline - full sequence PPL (question + answer)
        "baseline": {
        ...
        # 2. Baseline endings - PPL of answers only
        "baseline_endings": {
        ...
        # 4. Compression edge - PPL of (question + answer) with compression, excluding compression tokens from PPL
        "compression_edge": {
        ...
        # 5. Compression endings - PPL of answers only with compression tokens as context
        "compression_endings": {
        ...
        # 6. Compression only - PPL with compression tokens instead of original question
        "compression_only": {
        ...
        # 7. Compression only edge - PPL of answers only when replacing question with compression tokens
        "compression_only_edge": {
        ...
        # 8. Compression only endings - PPL of answers with compression tokens (alternative method)
        "compression_only_endings": {
    ...
    # Print summary
    print("\n" + "=" * 70)
    print(f"Evaluation Summary - ARC ({args.arc_subset})")
    print("=" * 70)
    ...
    print("\n" + "-" * 70)
    print("COMPRESSION-ONLY METHODS (compression tokens replace question)")
    print("-" * 70)

    print("\n6. Compression Only (compression tokens + answer):")
